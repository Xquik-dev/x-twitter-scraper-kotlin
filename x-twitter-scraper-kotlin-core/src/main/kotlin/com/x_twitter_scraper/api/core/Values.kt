...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw XTwitterScraperInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw XTwitterScraperInvalidDataException("`$name` is not set")
            is JsonNull -> throw XTwitterScraperInvalidDataException("`$name` is null")
            else -> throw XTwitterScraperInvalidDataException("`$name` is invalid, received $this")
        }
